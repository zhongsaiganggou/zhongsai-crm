  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_0.95fr]">
        ...
        <div className="space-y-5">
          <section className="surface p-5"><h3 className="font-semibold text-ink">负责人和状态</h3><div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2"><label><span className="label">客户状态</span><select className="field" value={data.currentStatusId} disabled={statusMutation.isPending} onChange={(event) => statusMutation.mutate(event.target.value)}>{statuses.data?.map((status) => <option key={status.id} value={status.id} disabled={status.code === 'INVALID' && status.id !== data.currentStatusId}>{status.nameZh}</option>)}</select></label>{user?.role === 'ADMIN' ? <label><span className="label">负责人</span><select className="field" value={data.assignedUserId ?? ''} disabled={assignMutation.isPending} onChange={(event) => event.target.value && assignMutation.mutate(event.target.value)}><option value="">待分配</option>{user.status === 'ACTIVE' && <option value={user.id}>{user.name}（管理员本人）</option>}{users.data?.filter((item) => item.role === 'SALES' && item.status === 'ACTIVE').map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label> : <DetailItem label="负责销售" value={data.assignedUser?.name} />}</div>{!data.requiresReview && data.qualityFlag !== 'CONFIRMED_INVALID' && <button className="btn-secondary mt-4 w-full" onClick={() => setReviewOpen(true)}><UserRoundCheck className="h-4 w-4" />核查线索质量</button>}</section>
